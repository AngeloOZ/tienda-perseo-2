...
export default function (req: NextApiRequest, res: NextApiResponse) {
  switch (req.method) {
    case 'GET':
      if (req.query?.id) {
        return obtenerProductoForId(req, res);
      }
...
const obtenerProductoForId = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
...
const registrarProducto = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    const { name, description, price, category, stock, cover, images_list, rating, status } =
      req.body as IProducto;
